...
export default function SlideViewer({ topicId, topicTitle, hasNextTopic, hasPrevTopic, startAtEnd, initialSlideIndex, nextTopicIsProject, nextTopicIsNewChapter, prevTopicIsNewChapter, onClose, onNextTopic, onPrevTopic, onCompleteTopic, onSlideChange }: SlideViewerProps) {
  ...
  }, [activeIndex, slides.length, topicId]);
    ...
    return (
      <div className="fixed inset-0 z-50 bg-white text-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <button 
            onClick={onClose} 
            className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 transition-colors rounded-lg font-medium text-white"
          >
            Volver al curso
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white text-slate-900 flex flex-col font-sans overflow-hidden">
          ...
          >
            ...
            <div className="flex gap-2 mb-8 md:mb-12">
              {slides.map((_, idx) => (
                <div 
                  key={idx} 
                  className={`h-1.5 rounded-full flex-1 max-w-[60px] transition-colors duration-300 ${idx <= activeIndex ? 'bg-indigo-600' : 'bg-slate-200'}`} 
                />
              ))}
            </div>
                        ...
                        {currentSlide.table.rows.map((row, rowIdx) => (
                          <tr key={rowIdx} className="hover:bg-slate-50/50">
                            {row.map((cell, cellIdx) => (
                              <td key={cellIdx} className={`px-4 py-3 ${cellIdx === 0 ? 'font-mono font-medium text-indigo-600' : 'text-slate-600'}`}>{cell}</td>
                            ))}
                          </tr>
                        ))}
                      </tbody>
              ...
              {currentSlide.cards && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8 lg:col-span-full">
                  {currentSlide.cards.map((card, idx) => {
                    ...
                    if (card.link) {
                      return (
                        <a key={idx} href={card.link} target="_blank" rel="noopener noreferrer" className="block h-full outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-xl">
                          {CardContent}
                        </a>
                      );
                    }
      ...
      <div className="p-4 md:p-6 flex items-center justify-between shrink-0 relative bg-white/90 backdrop-blur-sm border-t border-slate-100">
        ...
        <button 
          onClick={nextSlide}
          className={`flex items-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-lg font-medium transition-colors ${isLastSlide ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'text-slate-700 hover:bg-slate-100'}`}
        >
